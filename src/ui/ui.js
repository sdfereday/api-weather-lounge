...
export default ({
  ...
  useEffect(
    () => {
      setWeatherHex(
        weatherToHex[weatherData ? weatherData.main : weatherToHex[RAIN]]
      );
    },
    [weatherData]
  );
  ...
  return (
    <div
      className={`ui-wrapper transition-all ease-in-out`}
      style={{ backgroundColor: weatherHex }}
    >
      <div className="ui-gradient bg-gradient-to-t from-slate-800">
        <div className="ui-root w-[680px]">
          {children}
          <div className="text-center text-[#fefefe]">
            {weatherData
              ? <div className="summary-text">
                  ...
                  />
                  <p>
                    Current weather in {weatherData.location} appears to be{" "}
          ...
          <select
            ...
            onChange={manualWeatherChange}
            value={currentWeatherSelect}
          >
